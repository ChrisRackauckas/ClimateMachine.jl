...
function unpack_params(param_set::APS, rain_param_set::ARPS, ρ::FT,
    ...
    return (_n0, _r0, _m0, _me, _χm, _Δm, _a0, _ae, _χa, _Δa, _v0, _ve,_χv,_Δv)
end
...
"""
function lambda(q::FT, ρ::FT, n0::FT, m0::FT, me::FT, r0::FT, χm::FT,
                Δm::FT) where {FT<:Real}

    λ::FT = FT(0)

    if q > FT(0)
        λ = (χm * m0 * n0 * gamma(me + Δm + FT(1)) /
        ρ / q / r0^(me + Δm))^FT(1/(me + Δm + 1))
    end
    return λ
end
...
function terminal_velocity(param_set::APS, precip_param_set::APrecipPS,
    ...
    if q_ > FT(0)
        ...
        fall_w = _χv * _v0 * (_λ * _r0)^(-_ve -_Δv) *
                 gamma(_me + _ve + _Δm + _Δv + FT(1)) /
                 gamma(_me + _Δm + FT(1))
    end
...
function accretion_rain_sink(param_set::APS, ice_param_set::AIPS,
    ...
    if (q_ice > FT(0) && q_rai > FT(0))
        ...
        (_n0_rai, _r0_rai, _m0_rai, _me_rai, _χm_rai, _Δm_rai, _a0_rai,
         _ae_rai, _χa_rai, _Δa_rai, _v0_rai, _ve_rai, _χv_rai, _Δv_rai) =
            unpack_params(param_set, rain_param_set, ρ, q_rai)

        _E::FT  = E(ice_param_set, rain_param_set)

        _λ_ice::FT = lambda(q_ice, ρ, _n0_ice, _m0_ice,_me_ice,_χm_ice,_Δm_ice)
        _λ_rai::FT = lambda(q_rai, ρ, _n0_rai, _m0_rai,_me_rai,_χm_rai,_Δm_rai)

        accr_rate = _E / ρ * _n0_rai * _n0_ice * _m0_rai * _a0_rai * _v0_rai *
                    _χm_rai * _χa_rai * _χv_rai / _λ_ice / λ_rai *
                    gamma(_me_rai + _ae_rai + _ve_rai +
                          _Δm_rai + _Δa_rai + _Δv_rai + FT(1)) /
                    (_r0_rai * _λ_rai)^(_me_rai + _ae_rai + _ve_rai +
                                        _Δm_rai + _Δa_rai + _Δv_rai)
    end
    return accr_rate
end
...
function accretion_snow_rain(param_set::APS,
    ...
    if (q_i > FT(0) && q_j > FT(0))
        ...
        (_n0_i, _r0_i, _m0_i, _me_i, _χm_i, _Δm_i, _a0_i, _ae_i, _χa_i, _Δa_i,
         _v0_i, _ve_i, _χv_i, _Δv_i) =
          unpack_params(param_set, i_param_set, ρ, q_i)
        (_n0_j, _r0_j, _m0_j, _me_j, _χm_j, _Δm_j, _a0_j, _ae_j, _χa_j, _Δa_j,
         _v0_j, _ve_j, _χv_j, _Δv_j) =
          unpack_params(param_set, j_param_set, ρ, q_j)

        _E_ij::FT  = E(i_param_set, j_param_set)

        _λ_i::FT = lambda(q_i, ρ, _n0_i, _m0_i, _me_i, _χm_i, _Δm_i)
        _λ_j::FT = lambda(q_j, ρ, _n0_j, _m0_j, _me_j, _χm_j, _Δm_j)

        _v_ti = terminal_velocity(param_set, i_param_set, ρ, q_i)
        _v_tj = terminal_velocity(param_set, j_param_set, ρ, q_j)

        accr_rate = π / ρ * _n0_i * _n0_j * _m0_j * _χm_j* _E_ij *
                    abs(_v_ti - _v_tj) / _r0_j^(_me_j + _Δm_j) *
                    (
                      FT(2) * gamma(_me_j + _Δm_j + FT(1))
                        / _λ_i^FT(3) / _λ_j^(_me_j + _Δm_j+ FT(1)) +
                      FT(2) * gamma(_me_j + _Δm_j + FT(2))
                        / _λ_i^FT(2) / _λ_j^(_me_j + _Δm_j + FT(2)) +
                      gamma(_me_j + _Δm_j + FT(3))
                        / _λ_i /_λ_j^(_me_j + _Δm_j + FT(3))
                    )
    end
    return accr_rate
end
...
function evaporation_sublimation(param_set::APS, rain_param_set::ARPS,
    ...
    if (q_rai  > FT(0) && _S < FT(0))
        ...
        evap_subl_rate = FT(4) * π * _n0 / ρ * _S * _G / _λ^FT(2) * (
          _a_vent +
